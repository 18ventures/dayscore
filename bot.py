...
STOP_THRESHOLD_DEFAULT  = int(os.environ.get("STOP_THRESHOLD", "5"))
FIRST_LOSS_STOP_THRESHOLD = int(os.environ.get("FIRST_LOSS_STOP_THRESHOLD", "5"))  # tightened cap if trade #1 is a loss
...
DAILY_LOSS_LIMIT        = float(os.environ.get("DAILY_LOSS_LIMIT", "110"))       # max daily loss in $
DAILY_LOSS_STREAK_LIMIT = int(os.environ.get("DAILY_LOSS_STREAK_LIMIT", "3"))    # losses today before hard block. Rare-use override — set higher on Railway only when genuinely needed for a single day, then remove the var to return to 3.
...
POLL_INTERVAL           = int(os.environ.get("POLL_INTERVAL", "15"))             # seconds — fast loop, mark price/SL/TP monitoring
...
# ── Composite guard thresholds ───────────────────────────────────────────────
# No longs above this (extreme premium — don't buy the top)
COMPOSITE_LONG_MAX      = float(os.environ.get("COMPOSITE_LONG_MAX", "82"))
# No shorts below this (extreme discount — don't sell the bottom)
COMPOSITE_SHORT_MIN     = float(os.environ.get("COMPOSITE_SHORT_MIN", "18"))
# No shorts above this (already hyperextended premium — chasing too late)
COMPOSITE_SHORT_MAX     = float(os.environ.get("COMPOSITE_SHORT_MAX", "82"))
# No longs below this (already hyperextended discount — chasing too late)
# No longs below this (extreme discount — don't chase a falling knife)
COMPOSITE_LONG_MIN      = float(os.environ.get("COMPOSITE_LONG_MIN", "18"))

# ── Hyperliquid Mirror ────────────────────────────────────────────────────
HL_PRIVATE_KEY          = os.environ.get("HL_PRIVATE_KEY", "")
HL_ACCOUNT_ADDRESS      = os.environ.get("HL_ACCOUNT_ADDRESS", "")   # client wallet

# ── "All Signals" experiment — genuinely independent HL account ──────────
# Takes every INDICATOR_SIGNALS trade, no manual filtering, same guards as
# normal (composite/HTF/EMA), fixed $10 size — to isolate signal quality
# from Nathan's own discretionary filtering.
EXP_HL_PRIVATE_KEY      = os.environ.get("EXP_HL_PRIVATE_KEY", "")
EXP_HL_ACCOUNT_ADDRESS  = os.environ.get("EXP_HL_ACCOUNT_ADDRESS", "")
EXP_SIZE_USDT           = float(os.environ.get("EXP_SIZE_USDT", "10"))
EXP_LEVERAGE            = int(os.environ.get("EXP_LEVERAGE", "10"))
EXP_STOP_PCT            = float(os.environ.get("EXP_STOP_PCT", "0.5"))   # SL distance from entry %
EXP_TP_PCT              = float(os.environ.get("EXP_TP_PCT", "1.0"))    # TP distance from entry %
EXP_ENABLED             = os.environ.get("EXP_ENABLED", "false").lower() == "true"
HL_LEVERAGE             = int(os.environ.get("HL_LEVERAGE", "60"))
HL_SIZE_PCT             = float(os.environ.get("HL_SIZE_PCT", "70"))  # % of balance
HL_CLIENT_TG_TOKEN      = os.environ.get("HL_CLIENT_TG_TOKEN", "")
HL_CLIENT_CHAT_ID       = os.environ.get("HL_CLIENT_CHAT_ID", "")
...
WPR_LONG_MAX                = float(os.environ.get("WPR_LONG_MAX", "-35"))   # block longs if WPR above this
WPR_SHORT_MIN               = float(os.environ.get("WPR_SHORT_MIN", "-65"))  # block shorts if WPR below this
...
client_summary_sent_today   = False
client_trades_today: int    = 0
client_wins_today: int      = 0
client_losses_today: int    = 0
...
def get_hl_balance() -> float:
    """Fetch current HL account value for the client."""
    try:
        info  = hl_get_info()
        state = info.user_state(HL_ACCOUNT_ADDRESS)
...
MARK_PRICE_CACHE_SECONDS = 5  # short enough to never meaningfully affect real trade pricing,
                               # long enough to absorb multiple calls within the same poll cycle
...
def calculate_williams_r(highs: list, lows: list, closes: list, length: int) -> float:
    """
    Standard Williams %R over the most recent `length` candles.
...
_wpr_cache = {"value": None, "fetched_at": 0}
WPR_CACHE_SECONDS = 60  # a 30m WPR reading doesn't need re-fetching more often than this

def get_live_williams_r(interval: str = "30m", length: int = 25) -> float:
    """
    Fetches recent candles from Binance and calculates Williams %R on the
    LAST CLOSED candle — same closed-candle discipline as the HTF flip EMA
    functions, to avoid the live-candle whipsaw issue found earlier tonight.
    Returns the %R value, or None on failure.
    """
    now = time.time()
    if _wpr_cache["value"] is not None and (now - _wpr_cache["fetched_at"]) < WPR_CACHE_SECONDS:
        return _wpr_cache["value"]
    try:
        resp = requests.get(
        ...
        _wpr_cache["value"]      = wpr
        _wpr_cache["fetched_at"] = now
        return wpr
    except Exception as e:
...
_ema260_cache = {"value": None, "fetched_at": 0}
EMA260_CACHE_SECONDS = 120  # a 30m EMA can't meaningfully move within 2 minutes — safe to reuse

def get_live_ema260(interval: str = "30m") -> float:
    """
    Fetches recent candles from Binance and calculates the EMA(260) on demand —
    no dependency on any TradingView indicator. Fetches enough history for the
    EMA seed to have converged (well past the raw minimum).
    Cached briefly (EMA260_CACHE_SECONDS) since this can be called twice per
    signal fire (main guard + experiment guard) — without caching, a burst
    of signals firing in quick succession hammers Binance's kline endpoint
    hard enough to trigger IP-level rate limiting (418 errors), which then
    breaks every other Binance call the bot makes, including /stats.
    Returns the EMA value, or None on failure.
    """
    now = time.time()
    if _ema260_cache["value"] is not None and (now - _ema260_cache["fetched_at"]) < EMA260_CACHE_SECONDS:
        return _ema260_cache["value"]
    try:
        resp = requests.get(
            f"{FUTURES_BASE}/fapi/v1/klines",
        ...
        ema = calculate_ema(closes, 260)
        _ema260_cache["value"]      = ema
        _ema260_cache["fetched_at"] = now
        return ema
    except Exception as e:
...
GENERIC_EMA_CACHE_SECONDS = 120

def get_live_ema(interval: str, period: int) -> float:
    """
    Generic version of get_live_ema260 for arbitrary interval/period combos —
    used by signals that need EMAs get_live_ema260 doesn't cover (e.g. the
    4H 130 EMA / 30m 27 EMA combo for the HTF-flip-confirmation signal).
    Cached per (interval, period) key, same 2-minute logic as the dedicated
    130 EMA function — self-contained, no TradingView dependency.

    NOTE: this uses ALL closes including the still-forming current candle,
    so the returned EMA reflects the live, moving price. For genuine
    candle-close-based cross detection (see get_closed_candle_ema_and_close
    below), use that function instead — comparing live-price crosses
    produced repeated whipsaw false signals since the in-progress candle's
    price ticks back and forth across the EMA many times before it closes.
    """
...
_closed_candle_cache = {}  # keyed by (interval, period) -> {"ema": ..., "close": ..., "candle_open_time": ..., "fetched_at": ...}
CLOSED_CANDLE_CACHE_SECONDS = 60

def get_closed_candle_ema_and_close(interval: str, period: int) -> tuple:
    """
    Returns (ema, last_closed_price, candle_open_time_ms) using ONLY fully
    closed candles — drops Binance's still-forming current candle entirely,
    since comparing that live/moving price against the EMA is what caused
    the HTF flip signal to whipsaw (price ticks back and forth across the
    EMA repeatedly within a single candle before it actually closes).
    candle_open_time_ms lets the caller detect "is this a genuinely NEW
    closed candle since I last checked" rather than re-evaluating the same
    already-confirmed candle repeatedly.
    Returns (None, None, None) on failure.
    """
    cache_key = (interval, period)
    now = time.time()
    cached = _closed_candle_cache.get(cache_key)
    if cached and (now - cached["fetched_at"]) < CLOSED_CANDLE_CACHE_SECONDS:
        return cached["ema"], cached["close"], cached["candle_open_time"]
    try:
        resp = requests.get(
            f"{FUTURES_BASE}/fapi/v1/klines",
            params={"symbol": SYMBOL, "interval": interval, "limit": max(300, period * 3)},
        ...
        resp.raise_for_status()
        klines = resp.json()
        # Drop the last kline — Binance includes the still-forming current
        # candle as the final element, and we only want confirmed closes.
        closed_klines = klines[:-1]
        closes = [float(k[4]) for k in closed_klines]
        ema = calculate_ema(closes, period)
        last_closed_price     = closes[-1]
        last_closed_open_time = closed_klines[-1][0]
        _closed_candle_cache[cache_key] = {
            "ema": ema, "close": last_closed_price,
            "candle_open_time": last_closed_open_time, "fetched_at": now,
        }
        return ema, last_closed_price, last_closed_open_time
    except Exception as e:
        print(f"[EMA] Closed-candle calculation error ({interval}, period {period}): {e}")
...
def get_symbol_info() -> dict:
    ...
    for s in resp.json()["symbols"]:
        if s["symbol"] == SYMBOL:
            ...
            return info
    return {"qty_step": 0.001, "price_tick": 0.1}
...
def cancel_all_algo_orders():
    """Cancels every open algo (conditional) order for this symbol via the correct bulk endpoint."""
    now_ms  = int(time.time() * 1000)
    headers = {"X-MBX-APIKEY": BINANCE_API_KEY}
    params  = sign_request({"symbol": SYMBOL, "timestamp": now_ms})
    try:
...
# ── HTF-flip-confirmation short signal ────────────────────────────────────
# Fires when: (1) 30m price genuinely CROSSES below the 30m 27 EMA on a
# CONFIRMED candle close, AND (2) that same closed candle's price is also
# below the 4H 130 EMA (context — HTF trend starting to bend). Composite
# colour judgement is NOT automated — the prompt just asks Nathan directly,
# since "trending down" isn't a clean threshold the way a cross is.
#
# IMPORTANT: uses get_closed_candle_ema_and_close, not get_live_ema — the
# first version compared the LIVE, still-moving mark price against the EMA,
# which whipsawed repeatedly as price ticked back and forth across the EMA
# within a single still-forming 30m candle. Waiting for the candle to
# actually close before judging the cross fixes that at the root.
htf_flip_short_last_below_30m_70ema = None  # tracks previous CLOSED candle's state
htf_flip_short_last_evaluated_candle = None  # candle_open_time of the last candle we judged
HTF_FLIP_SHORT_ENABLED = os.environ.get("HTF_FLIP_SHORT_ENABLED", "false").lower() == "true"

# ── Aggressive taker-side volume detector — "shorts piling in" signal ────
# Uses Binance's aggTrades endpoint. Each trade is tagged isBuyerMaker (m):
# m=true means the SELLER was the aggressor (market sell hit the bid) —
# aggressive selling. m=false means the BUYER was the aggressor — aggressive
# buying. Over a rolling window, a heavy skew toward aggressive selling
# volume is a real-time signal of sellers forcing the issue, not just price
# drifting down passively.
AGGRESSIVE_VOLUME_ENABLED = os.environ.get("AGGRESSIVE_VOLUME_ENABLED", "false").lower() == "true"
AGGRESSIVE_VOLUME_WINDOW_MINS = int(os.environ.get("AGGRESSIVE_VOLUME_WINDOW_MINS", "5"))
AGGRESSIVE_VOLUME_SKEW_THRESHOLD = float(os.environ.get("AGGRESSIVE_VOLUME_SKEW_THRESHOLD", "70"))  # % of volume on one side to count as "piling in"
_last_aggressive_volume_check = 0.0
_last_aggressive_volume_alert_side = None  # avoids re-alerting the same ongoing skew repeatedly

def check_aggressive_volume():
    global _last_aggressive_volume_check, _last_aggressive_volume_alert_side
    if not AGGRESSIVE_VOLUME_ENABLED:
        return
    now = time.time()
    # Only actually check once per window, not every poll tick — a 5-min
    # window doesn't need re-evaluating every 15 seconds.
    if (now - _last_aggressive_volume_check) < (AGGRESSIVE_VOLUME_WINDOW_MINS * 60):
        return
    _last_aggressive_volume_check = now

    try:
        end_ms   = int(now * 1000)
        start_ms = end_ms - (AGGRESSIVE_VOLUME_WINDOW_MINS * 60 * 1000)
        buy_vol  = 0.0
        sell_vol = 0.0
        from_id  = None
        # Paginate defensively — a busy 5-min BTC window can exceed 1000
        # trades. Cap total requests so this can never spiral into a
        # runaway loop hammering the endpoint.
        for _ in range(10):
            params = {"symbol": SYMBOL, "startTime": start_ms, "endTime": end_ms, "limit": 1000}
            if from_id is not None:
                params = {"symbol": SYMBOL, "fromId": from_id, "limit": 1000}
            resp = requests.get(f"{FUTURES_BASE}/fapi/v1/aggTrades", params=params, timeout=10)
            resp.raise_for_status()
            batch = resp.json()
            if not batch:
                break
        ...
        if sell_pct >= AGGRESSIVE_VOLUME_SKEW_THRESHOLD:
            if _last_aggressive_volume_alert_side != "sell":
                send_telegram(
                    f"🔻 <b>Aggressive selling piling in</b>\n\n"
                    f"Last {AGGRESSIVE_VOLUME_WINDOW_MINS}m: {sell_pct:.0f}% of volume was "
                    f"aggressive sell-side (market sells hitting the bid).\n"
                    f"This is real-time pressure, not just a passive drift down."
                )
                db_log_warning("aggressive_volume", f"{sell_pct:.0f}% aggressive sell-side volume over {AGGRESSIVE_VOLUME_WINDOW_MINS}m")
            _last_aggressive_volume_alert_side = "sell"
        elif buy_pct >= AGGRESSIVE_VOLUME_SKEW_THRESHOLD:
            if _last_aggressive_volume_alert_side != "buy":
                send_telegram(
                    f"🔺 <b>Aggressive buying piling in</b>\n\n"
                    f"Last {AGGRESSIVE_VOLUME_WINDOW_MINS}m: {buy_pct:.0f}% of volume was "
                    f"aggressive buy-side (market buys hitting the ask).\n"
                    f"This is real-time pressure, not just a passive drift up."
                )
                db_log_warning("aggressive_volume", f"{buy_pct:.0f}% aggressive buy-side volume over {AGGRESSIVE_VOLUME_WINDOW_MINS}m")
            _last_aggressive_volume_alert_side = "buy"
        else:
            _last_aggressive_volume_alert_side = None  # back to balanced — reset so a future skew can alert again

    except Exception as e:
        print(f"[Aggressive Volume] Check error: {e}")


def check_htf_flip_short_signal():
    global htf_flip_short_last_below_30m_70ema, htf_flip_short_last_evaluated_candle
    if not HTF_FLIP_SHORT_ENABLED:
        return
    try:
        ema_30m_27, close_30m, candle_time_30m = get_closed_candle_ema_and_close(interval="30m", period=27)
        ema_4h_130, close_4h,  _               = get_closed_candle_ema_and_close(interval="4h",  period=130)
        if ema_30m_27 is None or ema_4h_130 is None:
            return

        # Only evaluate once per genuinely new closed 30m candle — otherwise
        # the same confirmed candle gets re-judged every poll until the next
        # one closes, which is harmless but pointless repeated work.
        if candle_time_30m == htf_flip_short_last_evaluated_candle:
            return
        htf_flip_short_last_evaluated_candle = candle_time_30m

        currently_below_30m_27ema = close_30m < ema_30m_27
        currently_below_4h_130ema   = close_4h < ema_4h_130

        # Detect a genuine CROSS down through the 30m 27 EMA — only fires on
        # the transition from "above/at" to "below" between one CLOSED
        # candle and the next, not on live intra-candle noise.
        just_crossed = (
            htf_flip_short_last_below_30m_70ema is False
            and currently_below_30m_27ema is True
        )
        htf_flip_short_last_below_30m_70ema = currently_below_30m_27ema

        if just_crossed and currently_below_4h_130ema:
            send_telegram(
                f"🌡️ <b>HTF flip confirmation — possible SHORT</b>\n\n"
                f"✅ 30m candle closed below the 27 EMA (${ema_30m_27:,.2f}, close ${close_30m:,.2f})\n"
                f"✅ Price is already closed below the 4H 130 EMA (${ema_4h_130:,.2f}) — HTF trend may be bending down\n\n"
                f"❓ <b>What colour/trend is the HTF composite right now?</b>\n"
                f"If it's Orange or lower, or clearly trending down — this may be "
                f"the real start of the HTF downtrend, not just a local dip.\n"
                f"If it's still Red/high — the bigger trend likely hasn't turned yet.\n\n"
                f"Use your own judgement on the composite before taking this."
            )
            db_log_warning("htf_flip_short_signal", f"30m candle closed below 130 EMA (${ema_30m_27:,.2f}, close ${close_30m:,.2f}) while already below 4H 130 EMA (${ema_4h_130:,.2f})")
    except Exception as e:
        print(f"[HTF Flip Short] Check error: {e}")


# ── HTF-flip-confirmation long signal (mirror of the short version) ──────
# Fires when: (1) 30m price genuinely CROSSES above the 30m 27 EMA on a
# CONFIRMED candle close, AND (2) that same closed candle is also above the
# 4H 130 EMA. Same composite judgement question, flipped.
htf_flip_long_last_above_30m_70ema = None
htf_flip_long_last_evaluated_candle = None
HTF_FLIP_LONG_ENABLED = os.environ.get("HTF_FLIP_LONG_ENABLED", "false").lower() == "true"

def check_htf_flip_long_signal():
    global htf_flip_long_last_above_30m_70ema
    global htf_flip_long_last_above_30m_70ema, htf_flip_long_last_evaluated_candle
    if not HTF_FLIP_LONG_ENABLED:
        return
    try:
        ema_30m_27, close_30m, candle_time_30m = get_closed_candle_ema_and_close(interval="30m", period=27)
        ema_4h_130, close_4h,  _               = get_closed_candle_ema_and_close(interval="4h",  period=130)
        ...
        if candle_time_30m == htf_flip_long_last_evaluated_candle:
            return
        htf_flip_long_last_evaluated_candle = candle_time_30m

        currently_above_30m_27ema = close_30m > ema_30m_27
        currently_above_4h_130ema   = close_4h > ema_4h_130

        # Detect a genuine CROSS up through the 30m 27 EMA between one
        # CLOSED candle and the next.
        just_crossed = (
            htf_flip_long_last_above_30m_70ema is False
            and currently_above_30m_27ema is True
        )
        htf_flip_long_last_above_30m_70ema = currently_above_30m_27ema

        if just_crossed and currently_above_4h_130ema:
            send_telegram(
                f"🌡️ <b>HTF flip confirmation — possible LONG</b>\n\n"
                f"✅ 30m candle closed above the 27 EMA (${ema_30m_27:,.2f}, close ${close_30m:,.2f})\n"
                f"✅ Price is already closed above the 4H 130 EMA (${ema_4h_130:,.2f}) — HTF trend may be bending up\n\n"
                f"❓ <b>What colour/trend is the HTF composite right now?</b>\n"
                f"If it's Green or lower, or clearly trending up — this may be "
                f"the real start of the HTF uptrend, not just a local bounce.\n"
                f"If it's still Aqua/low — the bigger trend likely hasn't turned yet.\n\n"
                f"Use your own judgement on the composite before taking this."
            )
            db_log_warning("htf_flip_long_signal", f"30m candle closed above 130 EMA (${ema_30m_27:,.2f}, close ${close_30m:,.2f}) while already above 4H 130 EMA (${ema_4h_130:,.2f})")
    except Exception as e:
        print(f"[HTF Flip Long] Check error: {e}")


def exp_check_guards(direction: str) -> tuple:
    """
    Standalone guard check for the experiment account — mirrors the same
    composite/HTF/EMA logic used in execute_trade(), but as a clean boolean
    check with no Telegram side-effects, so it can gate the experiment
    account without touching or risking the main manual trading path.
    Returns (passed: bool, reason: str).
    """
    # No trades on weekends — thin weekend liquidity, keep this a clean
    # systematic test rather than exposing it to weekend noise/wicks.
    now_uk = datetime.now(UK_TZ)
    if now_uk.weekday() >= 5:  # 5 = Saturday, 6 = Sunday
        return False, f"weekend — no experiment trades ({now_uk.strftime('%A')})"

    if direction == "long" and current_composite is not None and current_composite < COMPOSITE_LONG_MIN:
        return False, f"LTF composite too low ({current_composite:.1f})"
    if direction == "long" and current_composite is not None and current_composite > COMPOSITE_LONG_MAX:
        return False, f"LTF composite too high ({current_composite:.1f})"
    ...
    if direction == "long" and current_htf_composite is not None and current_htf_composite < 12:
        return False, f"HTF composite too low ({current_htf_composite:.1f})"
    if direction == "long" and current_htf_composite is not None and current_htf_composite > 88:
        return False, f"HTF composite too high ({current_htf_composite:.1f})"
    ...
    if EMA_GUARD_ENABLED:
        live_ema260 = get_live_ema260()
        if live_ema260 is not None:
            try:
                mark = get_mark_price()
            except Exception:
                mark = None
            if mark is not None:
                if direction == "long" and mark > live_ema260:
                    return False, f"price above 260 EMA (premium, ${live_ema260:,.2f})"
                if direction == "short" and mark < live_ema260:
                    return False, f"price below 260 EMA (discount, ${live_ema260:,.2f})"
...
def _execute_trade_inner(direction: str, size_pct: float = None, size_usdt: float = None,
    ...
    # Block longs when price is above the 260 EMA (buying a premium, chasing strength).
    # Block shorts when price is below the 260 EMA (selling a discount, chasing weakness).
    ...
    if EMA_GUARD_ENABLED:
        ...
        if live_ema260 is not None:
            ...
            if ema_check_price is not None:
                if direction == "long" and ema_check_price > live_ema260:
                    db_log_warning("ema_guard_block", f"Long blocked — price ${ema_check_price:,.2f} above 260 EMA (${live_ema260:,.2f})")
                    send_telegram(
                        f"🚫 <b>Long blocked — price above 260 EMA (premium)</b>\n\n"
                        f"Price: ${ema_check_price:,.2f}\n"
                        f"260 EMA: ${live_ema260:,.2f}\n\n"
                        f"This is chasing strength, not buying a pullback. Wait for price to "
                        f"come back to or below the 260 EMA before longing."
                    )
                    return
                if direction == "short" and ema_check_price < live_ema260:
                    db_log_warning("ema_guard_block", f"Short blocked — price ${ema_check_price:,.2f} below 260 EMA (${live_ema260:,.2f})")
                    send_telegram(
                        f"🚫 <b>Short blocked — price below 260 EMA (discount)</b>\n\n"
                        f"Price: ${ema_check_price:,.2f}\n"
                        f"260 EMA: ${live_ema260:,.2f}\n\n"
                        f"This is chasing weakness, not selling a bounce. Wait for price to "
                        f"come back to or above the 260 EMA before shorting."
                    )
                    return

    # ── Simple 27/260 EMA trade-flexibility guard ─────────────────────
    # Block LONG if price is above the 30m 27 EMA — designed to reduce how
    # often Nathan needs to check Binance manually, while still allowing
    # some trade flexibility rather than a full lockout. Block SHORT if
    # price is below the 30m 260 EMA. Two independent checks, different
    # EMAs, not a combined condition.
    if SIMPLE_EMA_GUARD_ENABLED:
        if direction == "long":
            ema_27 = get_live_ema(interval="30m", period=27)
            if ema_27 is not None:
                try:
                    check_price = get_mark_price()
                except Exception:
                    check_price = None
                if check_price is not None and check_price > ema_27:
                    db_log_warning("simple_ema_guard_block", f"Long blocked — price ${check_price:,.2f} above 30m 27 EMA (${ema_27:,.2f})")
                    send_telegram(
                        f"🚫 <b>Long blocked — price above 30m 27 EMA</b>\n\n"
                        f"Price: ${check_price:,.2f}\n"
                        f"27 EMA: ${ema_27:,.2f}"
                    )
                    return
        if direction == "short":
            ema_260 = get_live_ema(interval="30m", period=260)
            if ema_260 is not None:
                try:
                    check_price = get_mark_price()
                except Exception:
                    check_price = None
                if check_price is not None and check_price < ema_260:
                    db_log_warning("simple_ema_guard_block", f"Short blocked — price ${check_price:,.2f} below 30m 260 EMA (${ema_260:,.2f})")
                    send_telegram(
                        f"🚫 <b>Short blocked — price below 30m 260 EMA</b>\n\n"
                        f"Price: ${check_price:,.2f}\n"
                        f"260 EMA: ${ema_260:,.2f}"
                    )
                    return
    ...
    try:
        ...
        threading.Thread(target=hl_mirror_open, args=(direction, sl_price, tp1_price, tp2_price), daemon=True).start()
...
def _close_position_inner(reason: str = None) -> None:
    try:
        ...
        threading.Thread(target=hl_mirror_close, daemon=True).start()
...
def adjust_sl(new_sl_price: float) -> None:
    ...
    try:
        ...
        # ── Replace the REAL exchange-backed stop order, not just memory ──
        # Cancel the existing algo STOP_MARKET order (leaves TP LIMIT orders
        # alone, since those use a different endpoint), then place a new one
        # at the updated price via the correct algo order endpoint.
        cancel_all_algo_orders()
        try:
            place_algo_order(close_side, "STOP_MARKET", new_sl_price, close_position=True)
        except Exception as e:
            send_telegram(f"❌ <b>Failed to place new SL order</b>\n\n{e}\n\nYour position may be UNPROTECTED right now — check manually.")
...
def notify_client(text: str):
    if not HL_CLIENT_TG_TOKEN or not HL_CLIENT_CHAT_ID:
        return
    try:
        requests.post(
            f"https://api.telegram.org/bot{HL_CLIENT_TG_TOKEN}/sendMessage",
            json={"chat_id": HL_CLIENT_CHAT_ID, "text": text, "parse_mode": "HTML"},
            timeout=10
        )
    except Exception as e:
        print(f"[Client TG] Error: {e}")


def hl_is_configured() -> bool:
    return HL_SDK_AVAILABLE and bool(HL_PRIVATE_KEY) and bool(HL_ACCOUNT_ADDRESS)


def hl_get_exchange():
    account = EthAccount.from_key(HL_PRIVATE_KEY)
    return HLExchange(account, HL_BASE_URL, account_address=HL_ACCOUNT_ADDRESS)
...
def hl_get_balance() -> float:
    info = hl_get_info()
    state = info.user_state(HL_ACCOUNT_ADDRESS)
    print(f"[HL Mirror] Raw user state: {state.get('marginSummary', {})}")
...
def hl_calculate_size() -> float:
    try:
        balance    = hl_get_balance()
        mark_price = hl_get_mark_price()
        print(f"[HL Mirror] Balance: {balance}, Mark: {mark_price}")
        if mark_price <= 0 or balance <= 0:
            print(f"[HL Mirror] Size calc failed — balance={balance} mark={mark_price}")
            return 0.0
        margin   = balance * (HL_SIZE_PCT / 100)
        notional = margin * HL_LEVERAGE
        size_btc = notional / mark_price
        print(f"[HL Mirror] Size: {size_btc} BTC (margin={margin}, notional={notional})")
        return round(size_btc, 4)
    except Exception as e:
        print(f"[HL Mirror] Size calc error: {e}")
        send_telegram(f"⚠️ <b>HL Mirror size calc error</b>\n\n{e}")
        return 0.0


# ── "All Signals" experiment account — genuinely independent ─────────────
exp_pending_trigger_ids = {}   # tracks the open trigger_performance row id, keyed by coin

def exp_is_configured() -> bool:
    return HL_SDK_AVAILABLE and bool(EXP_HL_PRIVATE_KEY) and bool(EXP_HL_ACCOUNT_ADDRESS)


def exp_get_exchange():
    account = EthAccount.from_key(EXP_HL_PRIVATE_KEY)
    return HLExchange(account, HL_BASE_URL, account_address=EXP_HL_ACCOUNT_ADDRESS)


def exp_get_balance() -> float:
    info  = hl_get_info()
    state = info.user_state(EXP_HL_ACCOUNT_ADDRESS)
    return float(state["marginSummary"]["accountValue"])


def exp_get_open_position() -> dict:
    info  = hl_get_info()
    state = info.user_state(EXP_HL_ACCOUNT_ADDRESS)
    for p in state.get("assetPositions", []):
        pos_data = p.get("position", {})
        if pos_data.get("coin") == HL_COIN and float(pos_data.get("szi", 0)) != 0:
            return pos_data
    return None


def exp_calculate_size_btc(fixed_usdt: float) -> float:
    """Fixed $ margin size, converted to BTC quantity at current leverage."""
    try:
        mark_price = hl_get_mark_price()
        if mark_price <= 0:
            return 0.0
        notional = fixed_usdt * EXP_LEVERAGE
        size_btc = notional / mark_price
        return round(size_btc, 4)
    except Exception as e:
        print(f"[Experiment] Size calc error: {e}")
        return 0.0


def exp_open_trade(direction: str, sig_key: str):
    """
    Opens a trade on the genuinely independent experiment HL account, fixed
    $10 size, no filtering — fires on every INDICATOR_SIGNALS trigger that
    passes the same guards as normal execution. Tracked separately in
    trigger_performance under a distinct client_id so it never mixes with
    manual trade data.
    """
    if not EXP_ENABLED or not exp_is_configured():
        return
    try:
        # Only one position at a time on this account — skip if already in one
        existing = exp_get_open_position()
        if existing:
            print(f"[Experiment] Already in a position — skipping {sig_key}")
            send_telegram(
                f"🧪 <b>[Experiment] Signal skipped — {sig_key}</b>\n\n"
                f"Already in an open position on the experiment account. "
                f"One trade at a time — this signal was not taken."
            )
            return

        exchange = exp_get_exchange()
        size     = exp_calculate_size_btc(EXP_SIZE_USDT)
        if size <= 0:
            print("[Experiment] Could not calculate size — skipping")
            send_telegram(
                f"⚠️ <b>[Experiment] Could not open — {sig_key}</b>\n\n"
                f"Size calculation failed (likely a mark price fetch issue). "
                f"No trade opened."
            )
            return

        is_buy = direction == "long"
        exchange.update_leverage(EXP_LEVERAGE, HL_COIN, is_cross=True)
        ...
        if result.get("status") == "ok":
            ...
            row_id   = db_save_trigger(f"exp_{sig_key}", direction, entry_price=avg_px)
            exp_pending_trigger_ids[HL_COIN] = row_id
            emoji = "🟢" if is_buy else "🔴"

            # ── Place real SL/TP trigger orders on HL — the entry above was
            # naked otherwise, with nothing protecting the position between
            # signals. Rounded to whole dollars — safe default for BTC-perp
            # tick size, worth confirming against HL's actual tick on first
            # live fill.
            sl_line = ""
            tp_line = ""
            if avg_px > 0:
                if direction == "long":
                    sl_px = round(avg_px * (1 - EXP_STOP_PCT / 100))
                    tp_px = round(avg_px * (1 + EXP_TP_PCT / 100))
                else:
                    sl_px = round(avg_px * (1 + EXP_STOP_PCT / 100))
                    tp_px = round(avg_px * (1 - EXP_TP_PCT / 100))
                ...
                try:
                    sl_order_type = HLOrderType(trigger=HLTriggerOrderType(
                        triggerPx=sl_px, isMarket=True, tpsl="sl"
                    ))
                    sl_result = exchange.order(
                        HL_COIN, close_is_buy, size, sl_px, sl_order_type, reduce_only=True
                    )
                    if sl_result.get("status") == "ok":
                        sl_line = f"🛑 SL:    ${sl_px:,}\n"
                    else:
                        sl_line = "🛑 SL:    ⚠️ failed to place\n"
                        print(f"[Experiment] SL order failed: {sl_result}")
                except Exception as e:
                    sl_line = "🛑 SL:    ⚠️ failed to place\n"
                    print(f"[Experiment] SL order error: {e}")

                try:
                    tp_order_type = HLOrderType(trigger=HLTriggerOrderType(
                        triggerPx=tp_px, isMarket=True, tpsl="tp"
                    ))
                    tp_result = exchange.order(
                        HL_COIN, close_is_buy, size, tp_px, tp_order_type, reduce_only=True
                    )
                    if tp_result.get("status") == "ok":
                        tp_line = f"🎯 TP:    ${tp_px:,}\n"
                    else:
                        tp_line = "🎯 TP:    ⚠️ failed to place\n"
                        print(f"[Experiment] TP order failed: {tp_result}")
                except Exception as e:
                    tp_line = "🎯 TP:    ⚠️ failed to place\n"
                    print(f"[Experiment] TP order error: {e}")

            send_telegram(
                f"🧪 <b>[Experiment] Trade opened — {emoji} {direction.upper()}</b>\n\n"
                f"Signal:  {sig_key}\n"
                f"Size:    {size} BTC (${EXP_SIZE_USDT:.0f} fixed)\n"
                f"Entry:   ${avg_px:,.2f}\n"
                f"{sl_line}"
                f"{tp_line}\n"
                f"Independent account — takes every signal, no filtering."
            )
        else:
            print(f"[Experiment] Open failed: {result}")
            send_telegram(
                f"⚠️ <b>[Experiment] Order failed — {sig_key}</b>\n\n"
                f"Hyperliquid rejected the order.\nResult: {result}"
            )
    except Exception as e:
        print(f"[Experiment] Open error: {e}")
        send_telegram(f"⚠️ <b>[Experiment] Open error — {sig_key}</b>\n\n{e}")


def exp_close_trade(reason: str = "signal reversal"):
    """Closes the experiment account's open position, if any."""
    if not EXP_ENABLED or not exp_is_configured():
        return
    try:
        pos = exp_get_open_position()
        if not pos:
            return
        szi   = float(pos["szi"])
        ...
        # ── Cancel any lingering SL/TP trigger orders first — otherwise a
        # reduce-only order from this position could unexpectedly interact
        # with whatever opens next on this account.
        try:
            info = hl_get_info()
            open_orders = info.open_orders(EXP_HL_ACCOUNT_ADDRESS)
            for o in open_orders:
                if o.get("coin") == HL_COIN:
                    exchange.cancel(HL_COIN, o.get("oid"))
        except Exception as e:
            print(f"[Experiment] Order cancel error (non-fatal): {e}")

        result   = exchange.market_open(HL_COIN, side == "BUY", qty, None, 0.05)
        if result.get("status") == "ok":
            statuses   = result.get("response", {}).get("data", {}).get("statuses", [])
            avg_px     = float(statuses[0].get("filled", {}).get("avgPx", 0)) if statuses else 0
            closed_pnl = float(statuses[0].get("filled", {}).get("closedPnl", 0)) if statuses else 0
            row_id = exp_pending_trigger_ids.get(HL_COIN)
            if row_id:
                db_update_outcome(row_id, "win" if closed_pnl >= 0 else "loss", closed_pnl)
            emoji = "✅" if closed_pnl >= 0 else "❌"
            send_telegram(
                f"🧪 <b>[Experiment] Trade closed — {reason}</b>\n\n"
                f"Exit: ${avg_px:,.2f}\n"
                f"PnL:  {format_pnl(closed_pnl)}"
            )
    except Exception as e:
        print(f"[Experiment] Close error: {e}")


def hl_mirror_open(direction: str, sl_price: float = None, tp1_price: float = None, tp2_price: float = None):
    if not hl_is_configured():
        print("[HL Mirror] Not configured — skipping")
        return
    try:
        exchange = hl_get_exchange()
        size     = hl_calculate_size()

        if size <= 0:
            send_telegram("⚠️ HL Mirror: could not calculate size — skipping")
            return

        is_buy = direction == "long"

        exchange.update_leverage(HL_LEVERAGE, HL_COIN, is_cross=True)
        ...
        if result.get("status") == "ok":
            ...
            # Previously these prices were only ever displayed to the client
            # in a Telegram message — nothing was actually placed on the
            # exchange, meaning the client's position was completely naked,
            # protected by nothing but this bot's own polling loop noticing
            # and manually closing it. Same root-cause bug as the one found
            # and fixed on the main Binance account tonight — for a client's
            # real money, this is even more important to get right.
            close_is_buy = not is_buy  # closing side is opposite of entry
            ...
            if sl_price:
                try:
                    ...
                    if sl_result.get("status") == "ok":
                        ...
                        print(f"[HL Mirror] SL order failed: {sl_result}")
                except Exception as e:
                    real_sl_line = f"🛑 SL:  ${sl_price:,.2f} ⚠️ FAILED TO PLACE — verify manually\n"
                    print(f"[HL Mirror] SL order error: {e}")
            ...
            if tp1_price:
                try:
                    ...
                    if tp1_result.get("status") == "ok":
                        ...
                        print(f"[HL Mirror] TP1 order failed: {tp1_result}")
                except Exception as e:
                    real_tp1_line = f"🎯 TP1: ${tp1_price:,.2f} ⚠️ FAILED TO PLACE\n"
                    print(f"[HL Mirror] TP1 order error: {e}")
            ...
            if tp2_price:
                try:
                    ...
                    if tp2_result.get("status") == "ok":
                        ...
                        print(f"[HL Mirror] TP2 order failed: {tp2_result}")
                except Exception as e:
                    real_tp2_line = f"🎯 TP2: ${tp2_price:,.2f} ⚠️ FAILED TO PLACE\n"
                    print(f"[HL Mirror] TP2 order error: {e}")
            ...
            send_telegram(f"⚠️ <b>HL Mirror failed to open</b>\n\n{result}")
            print(f"[HL Mirror] Open failed: {result}")

    except Exception as e:
        send_telegram(f"⚠️ <b>HL Mirror error (open)</b>\n\n{e}")
        print(f"[HL Mirror] Open error: {e}")


def hl_mirror_close(max_retries: int = 4, retry_delay: int = 15):
    global client_trades_today, client_wins_today, client_losses_today
    if not hl_is_configured():
        return
    try:
        info  = hl_get_info()
        ...
        if not position:
            send_telegram("⚠️ <b>HL Mirror close</b> — no open position found on HL.")
            return
        ...
        print(f"[HL Mirror] Closing {close_qty} BTC — {'BUY' if is_buy else 'SELL'} — entry was ${entry_px:,.2f}")
        ...
        try:
            open_orders = info.open_orders(HL_ACCOUNT_ADDRESS)
            for o in open_orders:
                if o.get("coin") == HL_COIN:
                    exchange.cancel(HL_COIN, o.get("oid"))
        except Exception as e:
            print(f"[HL Mirror] Order cancel error (non-fatal): {e}")
        ...
        for attempt in range(1, max_retries + 1):
            ...
            print(f"[HL Mirror] Close attempt {attempt}/{max_retries} result: {result}")
            ...
            time.sleep(3)
            info2      = hl_get_info()
            state2     = info2.user_state(HL_ACCOUNT_ADDRESS)
            ...
            if not still_open:
                ...
                if closed_pnl == 0 and avg_px > 0 and entry_px > 0:
                    ...
                    print(f"[HL Mirror] closedPnl was 0/missing — calculated manually: ${closed_pnl:+.2f}")
                break  # confirmed closed — stop retrying

            if attempt < max_retries:
                send_telegram(
                    f"⚠️ <b>HL Mirror close attempt {attempt} failed — retrying in {retry_delay}s</b>\n\n"
                    f"Client position still open. Auto-retry {attempt}/{max_retries}."
                )
        ...
        if still_open:
            send_telegram(
                f"🚨 <b>HL Mirror close FAILED after {max_retries} attempts — UNPROTECTED POSITION</b>\n\n"
                f"Client's HL position is still open with no SL/TP watching it.\n"
                f"Close it manually NOW at app.hyperliquid.xyz\n\n"
                f"Client address: <code>{HL_ACCOUNT_ADDRESS}</code>"
            )
            notify_client(
        ...
        client_trades_today += 1
        if closed_pnl > 0:
            client_wins_today += 1
        elif closed_pnl < 0:
            client_losses_today += 1
        send_telegram(
            f"{emoji} <b>HL Mirror closed</b>\n\n"
            f"Exit:  ${avg_px:,.2f}\n"
            f"PnL:   ${closed_pnl:+.2f}"
        )
        notify_client(
        ...
        send_telegram(f"⚠️ <b>HL Mirror error (close)</b>\n\n{e}")
        print(f"[HL Mirror] Close error: {e}")


def execute_signal_trade(direction: str, label: str):
    """
    Execute a trade confirmed via Take Trade button.
    Checks time blocks and composite guards — if all clear, executes.
    """
    now_uk_full = datetime.now(UK_TZ)
    ...
    if direction == "short" and current_composite is not None and current_composite > COMPOSITE_SHORT_MAX:
        send_telegram(
            f"🚫 <b>Short blocked — composite hyperextended ({current_composite:.1f})</b>\n\n"
            f"Composite is above {COMPOSITE_SHORT_MAX:.0f} — move already overextended.\n"
            f"Shorts blocked to prevent chasing extended moves."
        )
        return
    ...
    if direction == "long" and current_composite is not None and current_composite < COMPOSITE_LONG_MIN:
        send_telegram(
            f"🚫 <b>Long blocked — composite hyperextended ({current_composite:.1f})</b>\n\n"
            f"Composite is below {COMPOSITE_LONG_MIN:.0f} — move already overextended.\n"
            f"Longs blocked to prevent chasing extended moves."
        )
        return
...
def poll_telegram_commands():
    global tg_last_update_id, daily_bias, pending_trigger_id, pending_signal, maker_entry_cancel_requested, pending_stress_check, pending_bias_reset, elevated_silence_until
        ...
        try:
            ...
            for update in updates:
                ...
                if cmd == "close":
                    ...
                    threading.Thread(
                        target=execute_trade,
                        kwargs=dict(direction="long", triggered_by="manual_long"),
                        daemon=True
                    ).start()
                elif cmd == "short":
                    threading.Thread(
                        target=execute_trade,
                        kwargs=dict(direction="short", triggered_by="manual_short"),
                        daemon=True
                    ).start()
                elif cmd == "cut1":
                    partial_close(25)
                    ...
                    try:
                        ...
                        send_telegram(
                            f"<b>HL Debug</b>\n\n"
                            f"Address: {HL_ACCOUNT_ADDRESS[:10]}...\n"
                        ...
                        send_telegram(f"HL Debug error: {e}")
                elif cmd == "stats":
                    pnl_summary()
                elif cmd == "patterns":
                    ...
                    if not rows and not avg_gaps:
                        ...
                        RETIRED_SIGNALS = {
                            ...
                            # 30m structure breaks — informational only
                        ...
                        try:
                            ...
                            if not rows:
                                ...
                                TRIGGER_LABELS = {
                                    ...
                                    "green_structure_break": "🟢 Green Structure Break",
                                    "orange_structure_break": "🟠 Orange Structure Break",
                                    "price_crossed_above_16_ema": "📈 Price Crossed Above 16 EMA",
                                    "price_crossed_below_16_ema": "📉 Price Crossed Below 16 EMA",
                                ...
                                lines = [
                                    ...
                                    f"<i>Fixed {VIRTUAL_TP_PCT}% TP / {VIRTUAL_SL_PCT}% SL per trigger, "
                                    f"regardless of whether traded</i>\n"
                                ]
                                for r in rows:
                    ...
                    if wpr is not None:
                        ...
                        send_telegram(
                            f"📐 <b>Williams %R Guard (30m, length 25)</b>\n\n"
                    ...
                    status_line = "🟢 Enabled" if AGGRESSIVE_VOLUME_ENABLED else "🔴 Disabled (set AGGRESSIVE_VOLUME_ENABLED=true)"
                    try:
                        ...
                        if total_vol > 0:
                            sell_pct = sell_vol / total_vol * 100
                            buy_pct  = 100 - sell_pct
                            lean = "🔻 Sell-side leaning" if sell_pct > 55 else ("🔺 Buy-side leaning" if buy_pct > 55 else "⚖️ Balanced")
                            send_telegram(
                                f"📊 <b>Order Flow — last {AGGRESSIVE_VOLUME_WINDOW_MINS}m</b>\n\n"
                                f"Status: {status_line}\n"
                                f"Threshold: {AGGRESSIVE_VOLUME_SKEW_THRESHOLD:.0f}%\n\n"
                                f"Aggressive buy:  {buy_pct:.1f}%\n"
                                f"Aggressive sell: {sell_pct:.1f}%\n\n"
                                f"{lean}"
                            )
                        else:
                            send_telegram(f"📊 Order Flow\n\nStatus: {status_line}\n\nNo trade data in this window.")
                    except Exception as e:
                        send_telegram(f"❌ Order flow check error: {e}")
                elif cmd == "htfflip":
                    short_status = "🟢 Enabled" if HTF_FLIP_SHORT_ENABLED else "🔴 Disabled"
                    long_status  = "🟢 Enabled" if HTF_FLIP_LONG_ENABLED else "🔴 Disabled"
                    ema_30m_27, close_30m, _ = get_closed_candle_ema_and_close(interval="30m", period=27)
                    ema_4h_130, close_4h,  _ = get_closed_candle_ema_and_close(interval="4h",  period=130)
                    try:
                        mark = get_mark_price()
                    except Exception:
                        mark = None
                    if ema_30m_27 is not None and ema_4h_130 is not None:
                        vs_30m = "🔴 Below" if close_30m < ema_30m_27 else "🟢 Above"
                        vs_4h  = "🔴 Below" if close_4h < ema_4h_130 else "🟢 Above"
                        mark_line = f"Live mark:      ${mark:,.2f}\n" if mark is not None else ""
                        send_telegram(
                            f"🌡️ <b>HTF Flip Signals — status</b>\n\n"
                            f"Short: {short_status}  (set HTF_FLIP_SHORT_ENABLED=true)\n"
                            f"Long:  {long_status}  (set HTF_FLIP_LONG_ENABLED=true)\n\n"
                            f"{mark_line}"
                            f"Last CLOSED 30m: ${close_30m:,.2f}  vs 27 EMA ${ema_30m_27:,.2f}  ({vs_30m})\n"
                            f"Last CLOSED 4H:  ${close_4h:,.2f}  vs 130 EMA ${ema_4h_130:,.2f}  ({vs_4h})\n\n"
                            f"Fires only on a CONFIRMED candle close crossing the 27 EMA "
                            f"while the same closed candle sits on the matching side of "
                            f"the 4H 130 EMA — down+below for short, up+above for long. "
                            f"Live mark price is ignored to avoid whipsaw."
                        )
                    else:
                        send_telegram(f"🌡️ <b>HTF Flip Signals</b>\n\nShort: {short_status}\nLong: {long_status}\n\nCould not fetch closed-candle EMA/price data.")
                elif cmd == "unmatched":
                    if not DATABASE_URL:
                        send_telegram("📊 No database connected.")
                    else:
                        ...
                        try:
                            ...
                            if not rows:
                                ...
                                CATEGORY_EMOJI = {
                                    ...
                                    "htf_extreme_block": "🌡️",
                                    "unrecognized_signal": "❓",
                                    "htf_flip_short_signal": "🌡️",
                                    "htf_flip_long_signal": "🌡️",
                                }
                    ...
                    if live_ema260 is not None:
                        try:
                            mark = get_mark_price()
                            if mark > live_ema260:
                                position_line = "🚫 Price ABOVE 260 EMA — longs blocked (premium)"
                            elif mark < live_ema260:
                                position_line = "🚫 Price BELOW 260 EMA — shorts blocked (discount)"
                            else:
                                position_line = "✅ Price at 260 EMA — both directions valid"
                            price_line = (
                                f"Price: ${mark:,.2f}\n"
                                f"260 EMA: ${live_ema260:,.2f}\n\n"
                                f"{position_line}\n\n"
                                f"Calculated live from Binance 30m candles — no TradingView dependency."
                            )
                        except Exception:
                    ...
                    status_line = "🟢 Enabled" if EXP_ENABLED else "🔴 Disabled (set EXP_ENABLED=true)"
                    configured_line = "✅ Configured" if exp_is_configured() else "❌ Missing EXP_HL_PRIVATE_KEY/EXP_HL_ACCOUNT_ADDRESS"
                    if not DATABASE_URL:
                        send_telegram(f"🧪 <b>All-Signals Experiment</b>\n\nStatus: {status_line}\n{configured_line}")
                    else:
                        try:
                            conn = get_db()
                            cur  = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
                            cur.execute("""
                                SELECT
                                    ...
                                    COUNT(*) FILTER (WHERE outcome = 'win') as wins,
                                    COALESCE(SUM(pnl) FILTER (WHERE outcome IN ('win','loss')), 0) as net_pnl,
                                    COALESCE(SUM(pnl) FILTER (WHERE outcome IN ('win','loss') AND entry_time >= NOW() - INTERVAL '7 days'), 0) as pnl_7d,
                                    COUNT(*) FILTER (WHERE outcome IN ('win','loss') AND entry_time >= NOW() - INTERVAL '7 days') as trades_7d
                                FROM trigger_performance
                                WHERE trigger LIKE 'exp_%%'
                            """)
                            r = cur.fetchone()
                            cur.close()
                            conn.close()
                            ...
                            balance_line = ""
                            try:
                                if exp_is_configured():
                                    bal = exp_get_balance()
                                    balance_line = f"Balance: ${bal:,.2f}\n"
                                    if bal == 0:
                                        balance_line += (
                                            f"⚠️ Balance shows $0 — double check EXP_HL_ACCOUNT_ADDRESS "
                                            f"on Railway matches the funded wallet exactly (address: "
                                            f"{EXP_HL_ACCOUNT_ADDRESS[:6]}...{EXP_HL_ACCOUNT_ADDRESS[-4:]})\n"
                                        )
                            except Exception as bal_err:
                                balance_line = f"⚠️ Balance fetch failed: {bal_err}\n"
                                print(f"[Experiment] Balance fetch error: {bal_err}")
                            if total > 0:
                                wr = (r["wins"] / total * 100)
                                send_telegram(
                                    f"🧪 <b>All-Signals Experiment</b>\n\n"
                                    f"Status: {status_line}\n"
                                    f"{configured_line}\n"
                                    f"{balance_line}"
                                    f"\nAll-time: {total} trades, {wr:.0f}% WR, {format_pnl(r['net_pnl'])} net\n"
                                    f"Last 7d: {r['trades_7d']} trades, {format_pnl(r['pnl_7d'])} net\n\n"
                                    f"Compare against your own /patterns and /quality to see if "
                                    f"unfiltered signal-taking beats your discretion."
                                )
                            else:
                                send_telegram(
                                    f"🧪 <b>All-Signals Experiment</b>\n\n"
                            ...
                            send_telegram(f"❌ Experiment stats error: {e}")
                elif cmd == "htf":
                    if current_htf_composite is not None:
                        color = composite_color(current_htf_composite)
                        send_telegram(
                            f"📊 <b>HTF Composite</b>\n\n"
                            f"Value: {current_htf_composite:.1f}\n"
                            f"Color: {color}"
                        )
                    else:
                        send_telegram("📊 HTF composite unknown — waiting for a composite_update alert.")
                elif cmd == "brute":
                    if not DATABASE_URL:
                        send_telegram("📊 No database connected.")
                    else:
                        try:
                            ...
                            conn.close()
                            if not rows:
                                send_telegram(f"🛡️ No brute-force closes in the last {days}d.")
                            else:
                                lines = [f"🛡️ <b>Brute-force closes — last {days}d</b> ({len(rows)})\n"]
                                for r in rows:
                                    ts    = r["logged_at"].strftime("%d %b %H:%M")
                                    BRUTE_EMOJI = {
                                        "hard_structure_invalidate": "🚨",
                                    ...
                                    emoji = BRUTE_EMOJI.get(r["category"], "🛡️")
                                    lines.append(f"{emoji} <b>{ts}</b> — {r['message']}")
                                send_telegram("\n\n".join(lines))
                        except Exception as e:
                    ...
                    signal_map = [
                        ...
                        ("price_crossed_below_16_ema", "short", "📉 Price Crossed Below 16 EMA", None),
                        ("price_crossed_above_16_ema", "long",  "📈 Price Crossed Above 16 EMA", None),
                    ...
                    send_telegram(
                        ...
                        "/wpr — Williams %R guard status (30m, length 25)\n"
                        "/flow — aggressive buy/sell volume, last 5m window\n"
                        "/htfflip — HTF flip short+long signal status (30m 27 EMA + 4H 130 EMA)\n"
                        "/unmatched — TradingView alerts that fired but matched no handler\n"
                        "/warnings — recent discipline warnings (default 24h)\n"
                        "/experiment — all-signals experiment account status\n"
...
def process_webhook_signal(data):
    ...
    INDICATOR_SIGNALS = {
        ...
        "short":                  ("short", "📉 Short"),
        "price_crossed_below_16_ema":  ("short", "📉 Price Crossed Below 16 EMA"),
        "price_crossed_above_16_ema":  ("long",  "📈 Price Crossed Above 16 EMA"),
        "dual_tf_ema_bullish_cross":   ("long",  "🚀 Dual-TF 16 EMA Bullish Cross (HTF+LTF)"),
        "dual_tf_ema_bearish_cross":   ("short", "🔻 Dual-TF 16 EMA Bearish Cross (HTF+LTF)"),
    ...
    if sig_key in INDICATOR_SIGNALS:
        ...
        # ── "All Signals" experiment account ────────────────────────────
        # Independent of every guard/flow below this point — takes every
        # signal that reaches here, gated only by exp_check_guards (mirrors
        # composite/HTF/EMA guards, no manual confirmation needed).
        # Any opposite-direction signal closes the open position first —
        # kept deliberately simple/mechanical, true to "no filtering."
        if EXP_ENABLED:
            try:
                exp_existing = exp_get_open_position()
            except Exception:
                exp_existing = None
            just_closed = False
            if exp_existing:
                exp_current_dir = "long" if float(exp_existing["szi"]) > 0 else "short"
                if exp_current_dir != direction:
                    exp_close_trade(reason=f"opposite signal fired ({sig_key})")
                    just_closed = True
            exp_passed, exp_reason = exp_check_guards(direction)
            if exp_passed:
                if just_closed:
                    time.sleep(2)  # let the close settle before opening the new position
                threading.Thread(target=exp_open_trade, args=(direction, sig_key), daemon=True).start()
            else:
                print(f"[Experiment] Skipped {sig_key} — {exp_reason}")
                send_telegram(
                    f"🧪 <b>[Experiment] Signal skipped — {label}</b>\n\n"
                    f"Reason: {exp_reason}\n\n"
                    f"No trade opened on the experiment account for this signal."
                )

        # ── HTF extreme guard for structure breaks ──────────────────────
        # Orange (short) blocked when HTF is deep in premium (>83) — don't
        # short into the strongest possible HTF bullish conviction.
        # Green (long) blocked when HTF is deep in discount (<13) — mirror,
        # don't long into the strongest possible HTF bearish conviction.
        if sig_key == "orange_structure_break" and current_htf_composite is not None and current_htf_composite > 83:
            db_log_warning("htf_extreme_block", f"Orange Structure Break blocked — HTF composite {current_htf_composite:.1f} (>83, deep premium)")
            send_telegram(
                f"🚫 <b>Orange Structure Break blocked — HTF too extreme ({current_htf_composite:.1f})</b>\n\n"
                f"HTF composite is above 83 — deep premium, strong bullish conviction on the higher timeframe.\n"
                f"Shorting here fights the strongest possible HTF trend. Skipping this signal."
            )
            return
        if sig_key == "green_structure_break" and current_htf_composite is not None and current_htf_composite < 13:
            db_log_warning("htf_extreme_block", f"Green Structure Break blocked — HTF composite {current_htf_composite:.1f} (<13, deep discount)")
            send_telegram(
                f"🚫 <b>Green Structure Break blocked — HTF too extreme ({current_htf_composite:.1f})</b>\n\n"
                f"HTF composite is below 13 — deep discount, strong bearish conviction on the higher timeframe.\n"
                f"Longing here fights the strongest possible HTF trend. Skipping this signal."
            )
            return
        ...
        VALID_LONG_TRIGGERS  = {"long", "strength", "green_structure_break", "price_crossed_above_16_ema", "30m_composite_crosses_up_30", "30m_composite_crosses_up_15", "wpr_crosses_up_82", "buy"}
        VALID_SHORT_TRIGGERS = {"3_bears", "bearish", "short", "orange_structure_break", "price_crossed_below_16_ema", "30m_composite_crosses_down_70", "30m_composite_crosses_down_85", "wpr_crosses_down_18"}
        ...
        # ── Counter-signal force close — dual confirmation ──────────────
        # Only force-close if the counter-signal AND the LTF composite are
        # BOTH pointing the wrong way at the same time — two independent
        # confirmations, not just one noisy signal. Uses the same 18/82
        # extreme thresholds as the normal entry guards.
        FORCE_CLOSE_ON_SHORT_SIGNALS = {"30m_crosses_up_4h_composite"}
        FORCE_CLOSE_ON_LONG_SIGNALS  = {"30m_crosses_down_4h_composite"}
        open_direction_cs = current_trade_entry.get("direction")
        counter_signal_fired = (
            (open_direction_cs == "short" and sig_key in FORCE_CLOSE_ON_SHORT_SIGNALS) or
            (open_direction_cs == "long"  and sig_key in FORCE_CLOSE_ON_LONG_SIGNALS)
        )
        composite_confirms = (
            (open_direction_cs == "short" and current_composite is not None and current_composite >= COMPOSITE_SHORT_MAX) or
            (open_direction_cs == "long"  and current_composite is not None and current_composite <= COMPOSITE_LONG_MIN)
        )
        if counter_signal_fired and composite_confirms:
            reason = (
                f"Dual confirmation while in a {open_direction_cs.upper()} — {label} fired "
                f"AND LTF composite already at {current_composite:.1f} (extreme zone)"
            )
            db_log_warning("counter_signal_close", reason)
            close_position_now(reason=reason)
            return
        elif counter_signal_fired:
            print(f"[Counter Signal] {label} fired in {open_direction_cs} but composite ({current_composite}) "
                  f"not yet at extreme — skipping force-close, single confirmation only")
...
def check_and_reset(trades: list):
    ...
    global daily_loss_warn_alerted, daily_loss_stop_alerted, profit_locked_today
    ...
    global client_summary_sent_today
    global client_trades_today, client_wins_today, client_losses_today
    ...
    if last_reset_day != today:
        ...
        overnight_auto_breakeven_done = False
        client_summary_sent_today   = False
        client_trades_today         = 0
        client_wins_today           = 0
        client_losses_today         = 0
...
client_tg_last_update_id = 0

def poll_client_commands():
    """Poll the client's Telegram bot for commands."""
    global client_tg_last_update_id
    if not HL_CLIENT_TG_TOKEN or not HL_CLIENT_CHAT_ID:
        return
    ...
    try:
        requests.post(
            f"https://api.telegram.org/bot{HL_CLIENT_TG_TOKEN}/deleteWebhook",
            json={"drop_pending_updates": True},
            timeout=10
        )
        # Short poll to grab the latest update_id and skip old updates
        resp = requests.get(
            f"https://api.telegram.org/bot{HL_CLIENT_TG_TOKEN}/getUpdates",
        ...
        if resp.ok:
            updates = resp.json().get("result", [])
            if updates:
                client_tg_last_update_id = updates[-1]["update_id"]
    except Exception as e:
        print(f"[Client TG] Init error: {e}")

    print("[Client TG] Command polling started.")
    while True:
        try:
            url    = f"https://api.telegram.org/bot{HL_CLIENT_TG_TOKEN}/getUpdates"
            params = {"timeout": 30, "offset": client_tg_last_update_id + 1, "allowed_updates": ["message"]}
            resp   = requests.get(url, params=params, timeout=40)
            resp.raise_for_status()
            updates = resp.json().get("result", [])

            for update in updates:
                client_tg_last_update_id = update["update_id"]
                msg     = update.get("message", {})
                chat_id = str(msg.get("chat", {}).get("id", ""))
                if chat_id != str(HL_CLIENT_CHAT_ID):
                    continue
                text = msg.get("text", "").strip().lower()
                if text == "/mystats":
                    try:
                        if client_trades_today == 0:
                            notify_client("📊 <b>Today's Stats</b>\n\nNo closed positions yet today.")
                        else:
                            wr        = (client_wins_today / client_trades_today * 100) if client_trades_today else 0
                            wr_emoji  = "✅" if wr >= 50 else "📉"
                            notify_client(
                                f"📊 <b>Today's Stats</b>\n\n"
                                f"Trades:    {client_trades_today}\n"
                                f"Results:   {client_wins_today}W / {client_losses_today}L\n"
                                f"{wr_emoji} Win Rate:  {wr:.0f}%\n"
                            )
                    except Exception as e:
                        print(f"[Client TG] /mystats error: {e}")

                elif text == "/balance":
                    try:
                        bal = hl_get_balance()
                        notify_client(f"💰 <b>Your balance</b>\n\n${bal:,.2f} USDC")
                    except Exception as e:
                        notify_client(f"❌ Balance error: {e}")
                        print(f"[Client TG] /balance error: {e}")

                elif text == "/status":
                    print(f"[Client TG] /status received from chat {chat_id}")
                    try:
                        info      = hl_get_info()
                        state     = info.user_state(HL_ACCOUNT_ADDRESS)
                        positions = state.get("assetPositions", [])
                        ...
                        if not pos:
                            notify_client("📊 No open position on your account.")
                        else:
                            szi        = float(pos["szi"])
                        ...
                        notify_client(f"❌ Status error: {e}")
                        print(f"[Client TG] /status error: {e}")
                    ...
                    try:
                        ...
                        if not has_pos:
                            notify_client("ℹ️ No open position to close.")
                        else:
                            notify_client("⏳ Closing your position...")
                            threading.Thread(target=hl_mirror_close, daemon=True).start()
                    except Exception as e:
                        notify_client(f"❌ Close error: {e}")
                        print(f"[Client TG] /close error: {e}")
...
def register_client_commands() -> None:
    """Register commands for the client's bot and clear any webhook conflicts."""
    if not HL_CLIENT_TG_TOKEN:
        return
    ...
    try:
        url  = f"https://api.telegram.org/bot{HL_CLIENT_TG_TOKEN}/setMyCommands"
        resp = requests.post(url, json={"commands": commands}, timeout=10)
        if resp.ok:
            print("[Client TG] Commands registered ✅")
        else:
            print(f"[Client TG] Command registration failed: {resp.text}")
    except Exception as e:
        print(f"[Client TG] Command registration error: {e}")
...
def register_telegram_commands() -> None:
    commands = [
        ...
        {"command": "flow",            "description": "Aggressive buy/sell volume, last 5m"},
        {"command": "htfflip",        "description": "HTF flip short signal status"},
        ...
        {"command": "experiment",   "description": "All-signals experiment account status"},
...
def run():
    ...
    global morning_brief_sent_today, weekly_digest_sent_today, weekly_okr_sent_today, breakeven_suggested, breakeven_last_suggested_at, bias_question_sent_today, daily_bias
    global daily_streaks_3plus
    global profit_locked_today, overnight_alerted_1015, overnight_alerted_1045, overnight_auto_breakeven_done
    global client_summary_sent_today
    global client_trades_today, client_wins_today, client_losses_today
    global entry2_placed_at
    ...
    tg_cmd_thread.start()
    client_cmd_thread = threading.Thread(target=poll_client_commands, daemon=True)
    client_cmd_thread.start()
    ...
    while True:
        try:
            ...
            if not first_loss_tightened_today:
                ...
                if positions_today and positions_today[0]["realizedPnl"] < 0:
                    ...
                    if STOP_THRESHOLD > FIRST_LOSS_STOP_THRESHOLD:
            ...
            # ── 11:30pm client daily summary ──────────────────────────
            is_1130 = now_uk.hour == 23 and now_uk.minute == 30
            if is_1130 and not client_summary_sent_today:
                client_summary_sent_today = True  # set BEFORE sending so exceptions can't re-trigger
                try:
                    if client_trades_today > 0:
                        wr        = (client_wins_today / client_trades_today * 100) if client_trades_today else 0
                        wr_emoji  = "✅" if wr >= 50 else "📉"
                    ...
                    # Sunday weekly summary for client
                    if now_uk.weekday() == 6:
                        weekly = load_last_7_days_stats()
                        if weekly:
                            total_closed = sum(d["closed_positions"] for d in weekly)
                            ...
                            notify_client(
                                f"📈 <b>End of Week</b>\n\n"
                                f"Today:     {client_trades_today} trades  |  {client_wins_today}W / {client_losses_today}L\n"
                                f"{wr_emoji} Win Rate:  {wr:.0f}%\n"
        ...
        # ── Slower periodic checks (WPR, HTF flip, aggressive volume, ────
        # virtual trigger resolution) — these don't need the fast SL/TP
        # polling cadence, so they run on the same SLOW_REFRESH_INTERVAL
        # (60s default) as trade history/balance, not every fast tick.
        ...
        if (now_for_slow_checks - _last_slow_checks_run) >= SLOW_REFRESH_INTERVAL:
            ...
            try:
                check_htf_flip_short_signal()
            except Exception as e:
                print(f"[ERROR] HTF flip short check: {e}")

            try:
                check_htf_flip_long_signal()
            except Exception as e:
                print(f"[ERROR] HTF flip long check: {e}")
            ...
            try:
                check_aggressive_volume()
            except Exception as e:
                print(f"[ERROR] Aggressive volume check: {e}")
